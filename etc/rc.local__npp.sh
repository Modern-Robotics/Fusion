...
#-------------------------------------------------------------------------------
# REVISION HISTORY:
# 05-Jun-19 <jwa> - Changed initial write to wifi log file to create rather than
#		            append.  We don't want this log file overtaking the user's 
#                   file space.
...
Mode_AccessPoint() {
    ...
    sudo cp ${MAIN_DIR}/etc/config-wap/wpa_supplicant.conf__npp.sh      /etc/wpa_supplicant/wpa_supplicant.conf
...
Mode_NetworkInterface() {
    ...
    sudo cp ${MAIN_DIR}/etc/config-nic/wpa_supplicant.conf__npp.sh      /etc/wpa_supplicant/wpa_supplicant.conf
