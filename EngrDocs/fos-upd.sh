#/!bin/bash
#===============================================================================
# fos-upd.sh - FusionOS Update Script - from Version 1.0.0 and above
#   This script is intended for command line use to update a Fusion to the 
#   current release candidate.
#-------------------------------------------------------------------------------
# Revision History
# 14-Apr-2019 <jwa> - This is a special version that has the Environment
#                     variable FRT_BRANCH hard coded to point to the current
#                     Release Candidate branch.  It is largely based on the
#                     update.sh script.
#
#===============================================================================
#
...
export FRT_BRANCH="FRT-v1.3.0bRC1b01"		# This is the Branch to install
...
# use it to drive the update   (NOTE: it must be set and exported from the
# command prompt to be available in this script.)
#
...
# -------------------------------------------------------------------
# Set the target path and check to see if it exists.
#
RUN_INSTALL=0
MAIN_DIR="/usr/Fusion"
...
if [[ !(-d ${MAIN_DIR}) ]] ; then
	...
	# Target exists, we are doing an update;  find the version info
...
if [[ ${FRT_BRANCH} ]] ; then
	...
	FOS_NEWVER=`sudo curl --silent --show-error https://raw.githubusercontent.com/Modern-Robotics/Fusion/master/version.txt`
fi
