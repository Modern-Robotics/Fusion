angular
    ...
    .factory('editorFactory', ['$http', 'appFactory', 'userFactory', 'socketFactory', function ($http, appFactory, userFactory, socketFactory) {
        ...
        // Sends Console input
        Editor.SendInput = function (data) {

            return $http.post('/api/programs/sendInput', data)
                .then(function (response) {
                    return response;
                }, function (error) {
                    appFactory.showToast(error.data, 'error');
        ...
        // Adds program to working files
        Editor.blocklyToWorkingFiles = function (data) {

            var workingFilesNameList = [];

            for (var i = 0; i < userFactory.User.workingEditorFiles.length; i++) {
                userFactory.User.workingEditorFiles[i].Selected = false;
                workingFilesNameList.push(userFactory.User.workingEditorFiles[i].Name);
            }

            var name = data.name.replace(".blk", "");
            var counter = "";
            var extension = ".py"


            while (workingFilesNameList.indexOf(name + counter + extension) > -1) {
                if (counter == "")
                    counter = 1
                else
                    counter++;
            ...
            userFactory.User.workingEditorFiles.push({
                ...
                Code: data.code,
                NeedsSaving: true,
                Filepath: userFactory.User.filepath + '/Editor/' + name + counter + extension,
                Selected: true,
                FileType: 'new',
                PreventUnsavedExit: false,
                BlocklyTransfer: true
            });
