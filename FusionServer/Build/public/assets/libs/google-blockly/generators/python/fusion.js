...
Blockly.Python['fusion_start'] = function (block) {
    if(virtualGamepad == 0) Blockly.Python.definitions_['fusion_setup'] = fusionLibraryReference;
    else if(virtualGamepad == 1) {
		if(virtual_camera == 0)Blockly.Python.definitions_['fusion_setup'] = 'import Fusion\nimport VirtualGamepad\nf = Fusion.driver()\nv = VirtualGamepad.service()';
		if(virtual_camera == 1){
			Blockly.Python.definitions_['fusion_setup'] = 'import Fusion\nimport VirtualGamepad\nf = Fusion.driver()\nv = VirtualGamepad.service(True, ('+ res_w +','+ res_h +'), '+ fps +', '+ drop_cam_mode +')';
			virtual_camera = 0;
		}
		virtualGamepad=0;
	}
...
Blockly.Python['fusion_VirtualGamepad_service'] = function (block) {
	virtual_camera = 1;
	virtualGamepad = 1;
	res_w = Blockly.Python.valueToCode(block, 'Res_w');
    res_h = Blockly.Python.valueToCode(block, 'Res_h');
    fps = Blockly.Python.valueToCode(block, 'fps');
...
Blockly.Python['fusion_VirtualGamepad_telemetry'] = function (block) {
	virtualGamepad = 1;
    var line_number = Blockly.Python.valueToCode(block, 'line');
